...
QString DownloadManager::saveFileName(const QUrl &url)
{
    ...
    QString standart_path = QStandardPaths::writableLocation(QStandardPaths::PicturesLocation);
    standart_path.append("/" + basename);

    /**
     * Check if the file name exists already, if so, append an increasing number and test again.
     */
    if (QFile::exists(standart_path)) {
        // already exists, don't overwrite
        int i = 0;
        standart_path += '.';
        while (QFile::exists(standart_path + QString::number(i)))
            ++i;

        standart_path += QString::number(i);
    }

    return standart_path;
}
...
void DownloadManager::startNextDownload()
{
    ...
    QAndroidJniObject::callStaticMethod<void>("turanmahmudov/wallpapers/WallpapersInJava",
                                                  "scanForPicture",
                                                  "(Ljava/lang/String;)V",
...
void DownloadManager::downloadFinished()
{
    ...
    QAndroidJniObject::callStaticMethod<void>("turanmahmudov/wallpapers/WallpapersInJava",
                                              "notify",
                                              "(Ljava/lang/String;)V",
                                              QAndroidJniObject::fromString("Download finished!").object<jstring>());
